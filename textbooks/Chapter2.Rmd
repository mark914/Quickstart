...
print(as.numeric(as.character(5/6)),digits = 12)
print(as.character(5/6))
6*as.numeric(as.character(5/6))
5/6 == as.numeric(as.character(5/6))
```
<small>(why is that last FALSE?)</small>
<small>(as.numeric 中有一个print(as.numeric(as.character(5/6)))函数,其中digits是一个缺省的值)</small>
<small>(处理浮点数的时候会出现问题)</small>
...
**Recycling** repeat elements in shorter vector when combined with longer(重复利用)
```{r}
x + c(-7, -8)
x ^ c(1, 0, -1, 0.5)
```
